<!-- <script>
  ...
  const navigateToProfile = (name) => {
    ...
    const isConnected = contacts.some(contact => contact.name.toLowerCase() === name.toLowerCase());
    const path = isConnected ? `/tp-profiles/${slug}/previous` : `/tp-profiles/${slug}/first`;
    goto(path);
  };
</script>
...
<script>
  ...
  onMount(async () => {
    if ($currentUser?.uid) {
      try {
        const thoughtPartnersRef = collection(db, 'users', $currentUser.uid, 'thoughtPartners');
        const querySnapshot = await getDocs(thoughtPartnersRef);
        contacts = querySnapshot.docs.map(doc => ({ slug: doc.id, ...doc.data() }));
      } catch (err) {
        error = err.message;
        console.error('Failed to load contacts:', err);
      }
  ...
  const navigateToReconnect = (slug) => {
    if (!slug) return;
    goto(`/reconnect/${slug}`);
  };
</script>
    ...
    {#if contacts.length > 0}
      <div class="tp-grid">
        {#each contacts as tp}
          <div
            class="tp-card"
            on:click={() => navigateToReconnect(tp.slug)}
            style="cursor: pointer;"
          >
            <img src={`/${tp.name}.png`} alt={tp.name} class="tp-avatar" />
            <span class="tp-name">{tp.name}</span>
